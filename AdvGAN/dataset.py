...
class Cycle_Dataset_tf(Dataset):
    def __init__(self, X_a, X_b, y_a=None, y_b=None, mask_y_b=None, augmentation=None, batch_size=64, n_core=4): # 'flip'
        """
        Class to build and use a dataset
    ...
    def transform(self, a, b, y):
        a = tf.clip_by_value(a + 0.002*tf.random_normal(tf.shape(a), 0, 0.1), -1.0, 1.0)
        b = tf.clip_by_value(b + 0.002*tf.random_normal(tf.shape(b), 0, 0.1), -1.0, 1.0)
        return a, b, y
...
class Dataset_simple(Dataset):
    ...
    def next_batch(self, size):
        ...
        X = self.X[self.cursor:self.cursor+size]
        y = self.y[self.cursor:self.cursor+size]
        self.cursor+= size
