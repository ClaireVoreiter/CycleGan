...
class CycleWGAN:
    ...
    def __init__(self):                      
        ...
        self.optimizer_C = tf.train.RMSPropOptimizer(learning_rate=5e-5)
    ...
    def build_model(self, reset_graph=False):
        ...
        if self.use_labels and not self.one_way:
            ...
            self.loss_classif_s = self.clf_s_alpha*tf.reduce_mean(-tf.reduce_sum(tf.one_hot(tf.argmax(self.classif_real_s, 1), shape_s_y[-1]) * tf.log(tf.clip_by_value(self.classif_fake_s_ ,1e-10,1.0)), reduction_indices=[1]))
            ...
            #y_target_mixed = (1-self.mask_t_y) * self.classif_fake_s + self.mask_t_y * self.y_t
            ...
            loss_classif_t_real_1 = tf.cond(self.use_Classifs, lambda: self.clf_t_alpha*(1/tf.maximum(1.0,tf.reduce_sum(1-self.mask_t_y)))*tf.reduce_sum(-tf.reduce_sum(((1-self.mask_t_y) * self.classif_fake_s) * tf.log(tf.clip_by_value(self.classif_real_t ,1e-10,1.0)), reduction_indices=[1])), lambda : 0.0 )       
            loss_classif_t_real_2 = tf.cond(self.use_Classifs, lambda: self.clf_t_alpha*(1/tf.maximum(1.0,tf.reduce_sum(self.mask_t_y)))*tf.reduce_sum(-tf.reduce_sum((self.mask_t_y*self.y_t) * tf.log(tf.clip_by_value(self.classif_real_t ,1e-10,1.0)), reduction_indices=[1])), lambda : 0.0 ) 
            loss_classif_t_real_3 = tf.cond(self.use_Classifs, lambda: self.clf_t_alpha*(1/tf.maximum(1.0,tf.reduce_sum(self.mask_t_y)))*tf.reduce_sum(-tf.reduce_sum((self.mask_t_y*self.y_t) * tf.log(tf.clip_by_value(self.classif_fake_t_ ,1e-10,1.0)), reduction_indices=[1])), lambda : 0.0 ) 
         
            # 
            self.loss_classif_t_real = 0.5*loss_classif_t_real_1 + loss_classif_t_real_2 + 0.25*loss_classif_t_real_3
            
            self.loss_classif_t = 0.5*self.loss_classif_t_fake + self.loss_classif_t_real
            
            self.accuracy_target = tf.reduce_mean(tf.cast(tf.equal(tf.cast(tf.argmax(self.y_t, axis=1), tf.float32), tf.cast(tf.argmax(self.classif_real_t, axis=1), tf.float32)), tf.float32))
                
            self.g_loss_s2t += 0.5 * self.loss_classif_t_fake    
            
            self.loss_classif_s_fake_labeled = self.clf_s_alpha*((1/tf.maximum(1.0,tf.reduce_sum(self.mask_t_y)))*tf.reduce_sum(-tf.reduce_sum((self.mask_t_y* self.y_t) * tf.log(tf.clip_by_value(self.classif_fake_s ,1e-10,1.0)), reduction_indices=[1])))   

            self.g_loss_t2s += 20*self.loss_classif_s_fake_labeled # frameRelou-man
        ...
        with tf.Session(config=config) as sess:
            ...
            if(restore):
                ...
                if exists:
                    ...
                    self.critic_losses = list(loader[0])
                    self.neg_losses = list(loader[1])
                    self.generator_losses = list(loader[2])
                    self.clf_losses = list(loader[3])
                else:
            ...
            ######################
            if not restore or not loaded:
                Data_X = tf.placeholder("float", (None, 64, 64, 13), name="Data_X")  
                Data_y = tf.placeholder("float", (None, 8), name="Data_y")
                
                y_pred = self.classifier_t(Data_X)
                
                C_var = [var for var in tf.trainable_variables() if "classifier" in var.name]
                
                ce = tf.reduce_mean(-tf.reduce_sum(Data_y * tf.log(tf.clip_by_value(y_pred ,1e-10,1.0)), reduction_indices=[1]))
                
                step = (self.optimizer_C).minimize(ce, var_list=C_var)      
                
                acc = tf.reduce_mean(tf.cast(tf.equal(tf.cast(tf.argmax(Data_y, axis=1), tf.float32), tf.cast(tf.argmax(y_pred, axis=1), tf.float32)), tf.float32))
